...
export function About() {
  return (
    <section id="about" className="container-px max-w-7xl mx-auto py-16">
      <motion.h2 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="font-spaceGrotesk text-3xl sm:text-4xl font-extrabold text-foreground opacity-90 mb-6 border-b border-foreground/10 pb-2 text-left">About</motion.h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="order-first md:order-none">
          <div className="relative mx-auto aspect-square w-full max-w-[360px] overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/5 shadow-soft">
